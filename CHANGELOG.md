...
- Admin **Sync Settings** card: choose the Nextcloud → Grafana push timing (async/sync) and enable a scheduled Grafana → Nextcloud pull with an interval (`15m`/`1h`/`1d`). Config only for now — read by the sync engine when it lands; injectable over `occ config:app:set grafana_sync <key>`.
