...
- **Deleting a dashboard file mirrors into Grafana, and the Nextcloud trash is the undo.** Trash a synced file and — with the new **Grafana recycle-bin folder** setting **off** (default) — its dashboard is deleted in Grafana right then (the full JSON is safe in the trashed file); restoring it re-creates the dashboard with a new id. Turn the setting **on** (naming a Grafana folder as the bin) and trashing instead **moves the dashboard into that folder, keeping its id** — restoring moves it back unchanged, and only **emptying the Nextcloud trash** deletes it for good (and only the items you cleared, never the whole bin). Trashing a **link** never deletes its dashboard; an untracked file is never touched. If Grafana can't confirm the delete, the trash is **aborted** so nothing desyncs.
- **Removing a folder mapping tears it down safely.** Deleting a mapping (admin panel or `occ grafana_sync:remove-mapping`) now **trashes its connected files** — so their dashboards follow the recycle-bin setting (deleted, or parked for a clean reconnect) — while **standalone files that were never part of the mapping are left strictly alone**. Restoring the trash (or re-mapping the folder) reconnects them.
