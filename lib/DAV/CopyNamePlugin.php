...
/**
 * Names a copied dashboard file OUR way before the copy happens, so it is never called
 ...
 * extension, because to it our file is a `.json` called `Fleet Health.grafana`:
 *
 *     Fleet Health.grafana (1).json     <- what the client asks for
 *     Fleet Health (1).grafana.json     <- what the file should be called
 *
 ...
 * the file is BORN as `Fleet Health (1).grafana.json`. It never exists under the other
 ...
 * {@see \OCA\GrafanaSync\BackgroundJob\ReconcileNameJob} still repairs it. Those are the
...
final class CopyNamePlugin extends ServerPlugin {
	...
	public function beforeCopy(RequestInterface $request, ResponseInterface $response): bool {
		try {
			...
			$this->logger->debug('grafana_sync: left a copy destination alone', [
