 ...
 * backend (mirrors the n8n master's StorageService, saga Ch2 §14.1):
 *
 ...
 *
 * Both paths: never create groups (the content groups are admin-managed); the
 * dashboard files carry the same metadata + tags written by {@see SyncService}.
 */
final class StorageService {
	 ...
	 * expressed nothing useful: it stopped no write to Grafana — the listeners and the
	...
	public function ensureFolder(Mapping $mapping, array|string|null $groups = null): Folder {
		...
		if ($mapping->useTeamFolder) {
			...
			$this->teamFolders->ensure($mapping->ncFolder, $wanted);
			return $this->teamFolders->getWritableFolder($mapping->ncFolder);
		}

		// Admin-owned backend.
		$uid = $this->teamFolders->resolveActorUid();
		$home = $this->rootFolder->getUserFolder($uid);
		$folder = $this->mkdirP($home, $mapping->ncFolder);
		if ($wanted !== null) {
		...
		try {
			if ($mapping->useTeamFolder) {
				return $this->teamFolders->isAvailable()
					? $this->teamFolders->contentGroups($mapping->ncFolder)
					: [];
			}
			...
			$this->logger->warning('grafana_sync: could not read the mapped folder\'s groups', [
	...
	private function syncGroupShares(Folder $folder, string $ownerUid, array $wanted): void {
		...
		foreach ($existing as $share) {
			...
			try {
				...
				$this->logger->warning('grafana_sync: failed to unshare from group', [
					'app' => Application::APP_ID,
					'group' => $gid,
					'exception' => $e,
				]);
		...
		foreach ($wanted as $gid) {
			...
			if ($share !== null) {
				if ($share->getPermissions() !== self::CONTENT_PERMISSIONS) {
					...
					try {
						...
						$this->logger->warning('grafana_sync: failed to update group share', [
							'app' => Application::APP_ID,
							'group' => $gid,
							'exception' => $e,
						]);
			...
			try {
				...
				$this->logger->warning('grafana_sync: failed to share with group (does it exist?)', [
					'app' => Application::APP_ID,
					'group' => $gid,
					'exception' => $e,
				]);
