...
final class StorageService {
	...
	public function ensureFolder(Mapping $mapping): Folder {
		if ($mapping->useTeamFolder) {
			...
			$this->teamFolders->ensure($mapping->ncFolder, $mapping->ncGroups, $mapping->mode);
			...
			if (isset($existing[$gid])) {
				...
				if ($share->getPermissions() !== $perms) {
					...
					try {
						...
						$this->logger->warning('grafana_sync: failed to update group share', ['exception' => $e, 'group' => $gid]);
					}
				}
				continue;
			}
