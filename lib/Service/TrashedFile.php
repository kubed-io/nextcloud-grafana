 ...
 *
 * Two closures rather than one because the two directions are both real: a dashboard
 * deleted out of the recycle-bin folder purges its trashed mirror, and a dashboard
 * rescued out of it brings the mirror back. Restoring the entry is what makes the
 * second one the SAME file rather than a fresh copy written beside a trashed original.
 */
final class TrashedFile {
	/**
	 * @param int $fileId the filecache id, unchanged by the trip through the trash —
	 *                    which is what makes the file's `grafana_*` metadata readable here,
	 *                    and what identifies the node again after a restore
	 * @param string $name the ORIGINAL basename, not the `.grafana.d<timestamp>` spelling
