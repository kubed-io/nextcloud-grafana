...
/**
 * Raises a Nextcloud notification (bell + toast) when a push to Grafana fails, so the
 * user who saved the file sees *what* happened and can fix the JSON. This is the native
 * channel for an async/background failure — we don't break the file save itself (that
 * would lose their edits) and we don't invent any UI.
 *
 * The notification is keyed on the file id so a later success can clear it
 * ({@see cleared}) and repeated failures collapse onto one entry instead of spamming
 * the bell. (The `link_edit_blocked` notice pairs with the DAV link-write guard and
 * lands in Course 4.)
 */
final class SyncNotifier {
	...
	/**
	 * Notify $userId that pushing $fileName (file id $fileId) to Grafana failed because
	...
	public function cleared(int $fileId): void {
		try {
			...
			$this->logger->debug('grafana_sync: could not clear push-failure notification', [
				'app' => Application::APP_ID,
				'exception' => $e,
			]);
		}
