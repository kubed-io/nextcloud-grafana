 ...
 * so the single-node stat returns `metadata-grafana_uid`.
 */
async function propfindGrafanaUid(node) {
  if (!node?.path) return ''
  try {
    ...
    return res?.data?.props?.['metadata-grafana_uid'] || ''
  } catch (e) {
...
async function resolveUrl(node) {
  return buildUrl(grafanaUrl, getGrafanaUid(node)) || buildUrl(grafanaUrl, await propfindGrafanaUid(node))
}
