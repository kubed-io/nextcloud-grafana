...
#
#   1. DUPLICATE STEP PATTERN (nextcloud-grafana, found here). Behat ignores the KEYWORD when
...
#
#   2. PARENTHESES IN A PLAIN-TEXT PATTERN (nextcloud-grafana, found here). Behat reads `(...)`
#      as an optional group, so `Grafana is not contacted (already deleted)` also matches
#      the bare `Grafana is not contacted` and collides with it. Reported as an ambiguous
...
# minutes across four legs and needs a live Nextcloud and a live Grafana to say the same.
...
for php in sorted(bootstrap.rglob('*.php')):
    for line in php.read_text(encoding='utf-8').splitlines():
        ...
        # as one. Escaped literally instead, `holds exactly 1 dashboard file(s)`
        # only matches text carrying the parentheses — and the live step
        # `"alpha-dash" holds exactly 1 dashboard file` gets reported as
        # undefined while Behat matches it perfectly well.
