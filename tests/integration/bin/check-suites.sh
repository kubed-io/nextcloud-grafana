...
#   a file in TWO suites    runs twice, in two legs, against two Grafana containers.
#                           Wasted minutes, and doubled odds of a flake.
#
# Neither shows up as a failure anywhere else, which is the whole argument for a
# text check that takes a second instead of a live Nextcloud and a live Grafana.
#
...
if [ "$fail" -ne 0 ]; then
  echo
  echo "Fix tests/integration/behat.dist.yml so the three suites partition features/."
  exit 1
fi
