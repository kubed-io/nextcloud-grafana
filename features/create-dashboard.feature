...
# for the "author in NC, live in Grafana" flow. LIVE: a .grafana.json written over WebDAV
# into a mapped folder fires NodeWrittenEvent → CreateInN8nListener → the dashboard
...
Feature: Create a dashboard from Nextcloud
  ...
  Scenario: New file in a mapped sync folder becomes a real dashboard
    Given a folder mapped as "sync" to the Grafana tag "nextcloud:demo"
    ...
    And the dashboard carries the "nextcloud:demo" tag
    And the file is stamped with the dashboard's "n8n_id"
