...
# never cost you data: `unmapped` files (moved out of a mapping, archived in Grafana —
# a standalone copy / template you kept), `ignored` files, and untracked `.grafana.json`
...
Feature: Purge the app's restorable files from Nextcloud
  ...
  Scenario: Purge deletes the synced file but leaves its dashboard in Grafana and the mapping intact
    ...
    Then no managed dashboard files remain in the "nextcloud:alpha" folder
  ...
  Scenario: Purge keeps an unmapped file — a standalone copy is never lost
    ...
    And a managed "sync" dashboard file in the "nextcloud:alpha" folder
    When the admin purges the Nextcloud files
    Then no managed dashboard files remain in the "nextcloud:alpha" folder
  ...
  Scenario: Sync from Grafana brings the file back after a purge
    ...
    Then the dashboard appears again as a file in the "nextcloud:alpha" folder
  ...
  Scenario: Purge keeps an ignored file
    Given a managed "ignored" dashboard file in the "nextcloud:alpha" folder
    When the admin purges the Nextcloud files
