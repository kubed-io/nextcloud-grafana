...
Thanks for taking the time to look at the security of **Grafana Sync**. This file describes
...
Use [GitHub's private vulnerability reporting](https://github.com/kubed-io/Grafana-sync/security/advisories/new)
...
- The version of Grafana Sync, the Nextcloud version, and the PHP version you tested on.
...
- The PHP backend in [`lib/`](lib/) (`OCA\GrafanaSync\…`).
- The JS frontend in [`src/`](src/) and its built bundle in `dist/`.
- The release tarball produced by [`publish.yml`](.github/dashboards/publish.yml).
- The CI dashboards in [`.github/dashboards/`](.github/dashboards/) when they could leak
...
- Grafana itself — report those to [Grafana's security team](https://docs.Grafana.io/hosting/securing/security/).
...
- **Grafana API key** — entered by the admin in the Nextcloud admin section; stored
  encrypted via `OCP\Security\ICrypto`. Never logged.
- **Grafana webhook bearer token** — same handling as the API key.
- **GitHub App private key** — used by the release dashboard to bypass branch protection
...
This app makes outbound HTTP requests to **one** destination: the Grafana instance an admin
...
**`allow_local_address => true`** (see [`lib/Service/GrafanaClient.php`](lib/Service/GrafanaClient.php)).
Nextcloud blocks requests to private/loopback/link-local addresses by default as an SSRF
guard; we opt out **on purpose**, because the target audience is self-hosters whose Grafana
typically lives at a private, in-cluster address (e.g. `http://Grafana.Grafana.svc:5678`) that the
default guard would refuse.

The trade-off, stated plainly for any audit:

- An actor who can set the Grafana base URL can point the server at an internal address
...
- The request surface is limited to the configured Grafana REST API (`X-N8N-API-KEY`, JSON) and the
  configured webhook path; responses are parsed as Grafana JSON, not reflected to the user.

If a deployment does **not** need local addresses (Grafana reachable on a public hostname), a future
