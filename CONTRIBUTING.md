...
Thanks for stopping by. This is **Grafana Sync** — a Nextcloud app that surfaces Grafana
dashboards as native files. It lives under the [kubed-io](https://github.com/kubed-io)
GitHub org, shares some dashboard plumbing with the rest of that org, and has a deliberate
...
| [lib/](lib/) | PHP backend (`OCA\GrafanaSync`): controllers, services, listeners, migrations, background jobs. |
| [src/](src/) | JS frontend source (Files row icon + "Open in Grafana" action). Built by Vite into `dist/`. |
...
| [.github/dashboards/](.github/dashboards/) | `pr.yml` (PR housekeeping), `tests.yml` (build + unit), `quality.yml` (audit + lint + Psalm), `publish.yml` (release tarball). |
...
5. **CI must pass.** All required dashboards green (see [What CI expects](#what-ci-expects)).
   This is a hard gate.
...
8. **Release** is a separate, manual step (`publish.yml` dashboard with `push: true`),
...
- **A feature file** in [`features/`](features/) — the behaviour in Gherkin, *first*. It's
  ...
  **live** (drop its `@todo`) in the same PR that lands its code. Keep the Gherkin DRY —
  reuse shared `Background` steps (e.g. `Given the app is connected to Grafana`) rather than
...
(docker-compose with NC + Grafana + db) is described in
...
target; deploy with `kubectl cp` (see [saga/Chapter_1_Mise_en_Place.md](saga/Chapter_1_Mise_en_Place.md)
...
2. **Build the frontend bundle** (only when JS changed):
   ...
   ```
   Output lands in `dist/grafana_sync-files.js`.
3. **Deploy into a running Nextcloud** (the app folder must appear under `custom_apps/`).
   The cluster path is documented in [saga/Chapter_1_Mise_en_Place.md](saga/Chapter_1_Mise_en_Place.md) §15.
...
test for the Grafana REST client.
...
Four dashboards run on PRs into `main` (and where it makes sense, on push to `main`):

| Dashboard | Trigger | Jobs | Must pass? |
|---|---|---|---|
| [`pr.yml`](.github/dashboards/pr.yml) (🔀 PR) | PR only | Auto-assign author + changelog check | yes |
| [`tests.yml`](.github/dashboards/tests.yml) (🧪 Tests) | PR + push to `main` | PHP unit (PHPUnit + JUnit report) + JS build | yes |
| [`quality.yml`](.github/dashboards/quality.yml) (🛡️ Quality) | PR + push to `main` | composer audit + php-cs-fixer + Psalm (→ SARIF) + ESLint + npm audit | yes |
| `CodeQL` (default setup) | PR + push to `main` | GitHub-managed JS/TS code scanning | yes |
| [`publish.yml`](.github/dashboards/publish.yml) (🧬 Publish) | manual `workflow_dispatch` | release tarball | n/a |

What the dashboards look for from your PR:
...
- **JS bundle builds** without errors and produces `dist/grafana_sync-files.js`.
...
Action versions in dashboards are kept current by Dependabot (when configured). If you're
editing a dashboard, **verify the action's latest major** with `gh api
repos/<owner>/<repo>/releases/latest` — the stale-major footgun is documented in
[Chapter 2 §5.2](saga/Chapter_2_Pretty_Package.md).

### Dashboard authoring conventions
...
- **Changelog**: every user-visible change adds an entry under `## [Unreleased]` in
  ...
  **The changelog is the release notes.** One line per entry — never a paragraph,
  ...
  - **Functional change** (a feature/behavior users notice, e.g.
    `- Publish a dashboard to Grafana from the file action.`) → the most detail you
...
- **Versioning**: SemVer. The release dashboard (`publish.yml`) bumps `package.json` and
  mirrors the version into `appinfo/info.xml` — you don't bump these in feature PRs.
- **Tags**: `v<major>.<minor>.<patch>`, applied by the release dashboard via
...
4. **Second run with `push: true`** to actually commit the bump, tag, and create the
   GitHub Release with `grafana_sync-<version>.tar.gz` attached.
...
- **"Why was it built this way?"** → [saga/Chapter_1_Mise_en_Place.md](saga/Chapter_1_Mise_en_Place.md)
